...
function kirim_email_order($namauser, $emailuser, $telpuser, $iduser, $usersaldo, $idpesanan, $totalharga, $almat_lengkap, $waktupesan, $waktukirim, $tipe_bayar) {
    ...
    /*--- Start SendMail to User ---*/
        ...
        Halo ".$namauser.",<br /><br /><br /><br />
        
        <b>
        Terimakasih atas kepercayaan Anda berbelanja di Ideasmart.<br /><br />
        Melaui email ini kami informasikan detail pesanan Anda.
        </b><br /><br />
        ...
        Jika Anda memiliki pertanyaan lainnya, silahkan menghubungi kami di nomor berikut <br /><br />
...
function auto_pesanan_batal($iduser){
    ...
    $args_datapesanan = "SELECT * FROM pesanan WHERE id_user='$iduser' AND status='5' AND aktif='1' AND status_kasir !='1'";
	...
	while ( $item_pesanan = mysqli_fetch_array($result_datapesanan) ) {
	    ...
	    $tipebayar = $item_pesanan['tipe_bayar'];
	    
	    if($tipebayar == pay_debit){
	       // Cek data request saldo
    	    ...
    	    while ( $item_reqsaldo = mysqli_fetch_array($result_reqsaldo) ){
        	    ...
        	    if($count_konfrimsaldo > 0){
        	        ...
        	        $date_timeout = ($item_pesanan['waktu_pesan'] * 1) + $waktu_total;
        	        $date_timeout2 = 'n * 1jam';
        	        //Pengecekan Timeout
...
function parcelstock($data_id,$n){
	global $dbconnect;
	...
	while ( $sum_number <= $number_parcelprod ) {
			$idproduk               = $get_array_nama[$sum_number];
			$id[]                   = $get_array_nama[$sum_number];
            $total_parcel_jumlah    = $get_array_jumlah[$sum_number];
            $nkebutuhan             = $total_parcel_jumlah * $n;
            $filterawal    = all_stok_prod($idproduk,'stock_tersedia');
            if($filterawal < $nkebutuhan){
                $kode[] = 0;
            }else{
                $kode[] = 1;
    ...
    $countid    = count($id);
    $sumkodestok = array_sum($kode);
    if ($sumkodestok < $countid) {
    	$nitem    = 0;

    } else {
        $nitem    = all_stok_prod($data_id,'stock_tersedia');
    }
    return $nitem;
}
...
//Get stock produk_item/parcel from master to know its 
function parcelformaster($data_id){
	global $dbconnect;
	$args = "SELECT * FROM produk_item WHERE id_prod_master='$data_id'";
	$query = mysqli_query( $dbconnect, $args);
	$array_check = mysqli_fetch_array($query);
	...
	while ( $sum_number <= $number_parcelprod ) {
            ...
            $total_parcel_jumlah    = $get_array_jumlah[$sum_number];
            $filterawal    = all_stok_prod($idproduk,'stock_tersedia');
            if($filterawal < $total_parcel_jumlah){
                $kode[] = 0;
            }else{
                $kode[] = 1;
