...
if ( isset($_POST['hitungstock']) && $_POST['hitungstock']==MOBILE_FORM ) {
	...
	$upstock = "INSERT INTO pesanan ( id_user, telp, idproduk, jml_order, harga_item, sub_total, total, alamat_kirim, waktu_pesan, waktu_kirim, metode_bayar, tipe_bayar, status) 
	                         VALUES ( '$id_user', '$telp_user', '$id_jadi', '$stock_jadi', '$harga_jadi', '$cart_total_harga', '$cart_total_harga','$alamat_order', '$tgl_database','$order_date','$metode_bayar','$pay_method', '$status')";
	...
	if ( $metode_bayar == 'nonsaldo' ) {
	    ...
	    $args_reqsaldo = "INSERT INTO request_saldo (id_user,harga_awal,total_bayar,date_checkout,type,id_pesanan,status,aktif,id_trans_saldo) VALUES ('$id_user','$cart_total_harga','$cart_total_harga','$tgl_database','pesanan','$id_pesanan','1','1','0')";
...
if ( isset($_POST['open_riwayat']) && $_POST['open_riwayat']==MOBILE_FORM ) {
	...
	if( $status == '60' ) {
	    ...
	    $stat = "status LIKE '%$status%'";
	}
	$args="SELECT * FROM pesanan WHERE $stat AND id LIKE '%$idpesanan%' AND id_user='$user_id'";
    $result = mysqli_query( $dbconnect, $args );
	$riwayat = array();
	$jumlah = mysqli_num_rows($result);
	if ( $result ) {
...
if ( isset($_POST['detil_riwayatorder']) && $_POST['detil_riwayatorder']==MOBILE_FORM ) {
	...
	if( $result_pesanan && $result_pembayaran) {
...
if ( isset($_POST['konfirmtrans']) && $_POST['konfirmtrans']==MOBILE_FORM ) {
	...
	$args = "INSERT INTO konfirmasi_saldo ( tanggal, uang_saldo, uang_tf, rekuser, bankuser, namauser, rekideasmart, iduser, id_reqsaldo ) VALUES ( '$tanggal_db', '$nominal_trans', '$uang_trans', '$norek_user', '$bankuser_user', '$namarek_user', '$idsm_rek', '$id_user', '$id_reqsaldo')";
    $result = mysqli_query( $dbconnect, $args );
