...
if(is_admin()){
	...
	if( isset($_GET['date_from']) && isset($_GET['date_to']) ){
		...
		$args = "SELECT * FROM hapiut WHERE type='debt' AND aktif='1' AND date >= '$tgl_from' AND date <= '$tgl_to'";
		$result = mysqli_query($dbconnect,$args);

		$judul = "Laporan Hutang ".$datefrom." - ".$dateto; 
		...
		$objPHPExcel->getProperties()->setCreator("www.Akun.pro")
			...
			->setCategory( _('LAPORAN Hutang - ').$datefrom." - ".$dateto );
...
$objPHPExcel->getActiveSheet()->setCellValue('C'.$row, _("Klien") );
...
$objPHPExcel->getActiveSheet()->setCellValue('D'.$row, _("Tanggal Hutang") );
...
$objPHPExcel->getActiveSheet()->setCellValue('E'.$row, _("Hutang Awal") );
...
$objPHPExcel->getActiveSheet()->setCellValue('F'.$row, _("Hutang Sekarang") );
...
$objPHPExcel->getActiveSheet()->setCellValue('H'.$row, _("Status") );
$objPHPExcel->getActiveSheet()->getStyle('H'.$row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);


$objPHPExcel->getActiveSheet()->getStyle('B'.$row.':'.$alpha_end.$row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
...
$objPHPExcel->getActiveSheet()->getStyle('B'.$row.':'.$alpha_end.$row)->getFont()->setBold(true);
